...
#include <strings.h> /* for bcopy(), bsd */
#include <string.h> /* for memcpy(), posix, whoo hoo */
...
/* cs551 fa2011 code begin */
/* next lines set byte order on solaris for 551, by hand.
 * Unfortunately no POSIX standard. */
...
#endif
#ifndef BYTE_ORDER
#if defined(sparc) || defined(__sparc) || defined(SPARC) || defined(__SPARC)
...
/* cs551 fa2011 code begin */
unsigned int
projb_hash(unsigned char *buffer, int buffer_length)
{
	SHA1_CTX ctx;
	...
	SHA1Init(&ctx);
	SHA1Update(&ctx, buffer, buffer_length);
