		...
		for (i = 0; i <= sockMax; i++) {
			if (FD_ISSET(i, &readfds)) {
				if (i == nSockmgr) {		// new connection
					...
					if ((newsock = accept(nSockmgr,
							(struct sockaddr *) &sinWkraddr, &nsinSize))
							== -1) {
						...
						if (SendStreamData(newsock, szSendbuf, nBytestosend)
								< 0) {
							printf(
									"projb manager error: send nonce name etc error! Close socket!\n");
							close(newsock);
							continue;
						}

						// recv modified nonce and port number
						if ((nRecvbytes = RecvStreamLineForSelect(newsock,
								szRecvbuf, nRecvbufsize)) < 0) {
							printf(
									"projb manager error: receive client message error! Close socket!\n");
							close(newsock);
							continue;
			...
			for (i = 0; i <= sockMax; i++) {
				if (FD_ISSET(i, &readfds)) {
					if (i == nSockmgr) {   // not possible
						printf(
								"projb manager: listen socket got an unexpected connection, ignore!\n");
					} else if (i == CNameHead->tcpsock) { // possibly first node finishes its job, good!
						// but check first
						if ((nRecvbytes = RecvStreamLineForSelect(i, szRecvbuf,
								nRecvbufsize)) < 0) {
							errexit(
						...
						if (strcmp(szRecvbuf, "ok") != 0) {
							printf(
									"projb manager exceptioin: recv unknown reply form first node: %s\n",
									szRecvbuf);
							nExit = 1;
							break;
						}
						...
						if (curjob->jobtype == STRJOB) {    // store
							snprintf(szSendbuf, sizeof(szSendbuf),
									"store\n%s\n", pstPos->txt);
							nBytestosend = strlen(szSendbuf);
							if (SendStreamData(CNameHead->tcpsock, szSendbuf,
									nBytestosend) < 0) {
							...
							int k;
							for (k = 0; k < nClient; k++) {
								if (strcmp(tempcn->namestr, pendclPos->namestr)
										== 0)
									break;
